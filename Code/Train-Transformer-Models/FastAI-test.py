...
learner.load(leaner_name_save)


def get_preds_and_labelsBEA(dfin):
    ...
    y = np.array(dfin[Y_COLUMN].tolist())
    best_preds = np.argmax(test_preds, axis=1)
    all_preds_ranked = np.argsort(-test_preds, axis=1)

    fun = lambda t: learner_mapping[t]
    vfunc = np.vectorize(fun)
    all_preds_ranked = vfunc(all_preds_ranked)
    best_preds = vfunc(best_preds)
...
def compute_accuracy(real_labels, best_preds, all_preds):
    ...
    acc_at_toal = np.mean([1 if r in a else 0 for (r,a) in zip(real_labels, all_preds)]) if all_preds is not None else -1
    return np.round(acc_1,6), np.round(acc_3,6), np.round(acc_5,6), np.round(acc_10,6), np.round(acc_at_toal,6)


# In[100]:

best_preds, all_preds_ranked, y = get_preds_and_labelsBEA(test)
